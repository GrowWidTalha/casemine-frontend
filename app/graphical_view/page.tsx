...
import { FileText, BookOpen, Scale, Gavel, ArrowRight, ArrowLeft } from "lucide-react"
import { CitationGraph, type ApiResponse as CitationApiResponse } from "@/components/citationNetworkGraph"
        ...
        return (
            <div className="space-y-6">
                <div className="bg-purple-50 p-4 rounded-md">
                    <h2 className="text-xl font-bold mb-2 text-purple-900">{document.filename}</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <Card>
                        ...
                        <CardContent className="pt-4">
                            <div className="text-sm">{formatText(document.petitioner_arguments)}</div>
                        </CardContent>
                    </Card>
                    ...
                    <Card>
                        ...
                        <CardContent className="pt-4">
                            <div className="text-sm">{formatText(document.respondent_arguments)}</div>
                        </CardContent>
                    </Card>
                </div>
                    ...
                    {relevantData.nodes.map((doc, index) => (
                        ...
                        >
                            ...
                            <div
                                ...
                                dangerouslySetInnerHTML={{ __html: doc.snippet.substring(0, 150) + "..." }}
                            />
                            <div className="flex gap-2 mt-2">
                                <Badge variant="outline">Similarity: {(doc.similarity_score * 100).toFixed(1)}%</Badge>
                                {doc.year && doc.year !== "Unknown" && <Badge variant="outline">Year: {doc.year}</Badge>}
                            </div>
                        </div>
                    ))}
                </div>
                ...
                <div className="space-y-4">
                    {relevantData.relevant_documents.map((doc, index) => (
                        ...
                        >
                            ...
                            <div className="flex gap-2 mt-2">
                                <Badge variant="outline">Cites: {doc.citation_details.cites.length}</Badge>
                                <Badge variant="outline">Cited by: {doc.citation_details.cited_by.length}</Badge>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                return (
                    <div className="border rounded-lg p-4 h-[calc(100vh-200px)] overflow-y-auto">
                        <div className="flex items-center justify-center h-full">
                            <p className="text-gray-500">Citations are now displayed on the right side panel</p>
                        </div>
                    </div>
                )
            default:
            ...
            <div className="max-w-6xl mx-auto p-4 bg-white">
                ...
                <div className="flex flex-wrap justify-between gap-4 mb-8">
                    {filterCategories.map((category) => (
                        <div
                            key={category.id}
                            className="flex-1 bg-[#fdcc55] text-black px-4 py-3 rounded-md font-medium text-center shadow-md"
                        >
                            {category.label}
                        </div>
                    ))}
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                    {subFilters.map((filter) => (
                        <button
                            key={filter.id}
                            className={`${activeTab === filter.id ? "bg-[#4D2DB7]" : "bg-[#5D3FD3]"
                ...
                <div className="flex flex-col md:flex-row gap-6">
                    ...
                    <div className="w-full md:w-2/5">
                        <div className="bg-[#E4DDFF] p-6 rounded-sm h-full overflow-y-auto">
                            <h3 className="text-center text-black font-bold mb-4">Citations Found</h3>
                            {renderCitationsList()}
                        </div>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2 mt-8">
                    {bottomFilters.map((filter) => (
                        <button key={filter.id} className="bg-[#5D3FD3] text-white px-6 py-2.5 rounded-md font-medium relative">
                            {filter.label}
                            {filter.subLabel && (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                    <div>
                        <h2 className="font-bold text-2xl mb-4">{contentSections[0].title}</h2>
                        <p className="text-sm text-gray-800 leading-relaxed">{contentSections[0].content}</p>
                    </div>
                    <div className="flex items-center justify-center overflow-hidden">
                        <Image src={balanceimg || "/placeholder.svg"} alt="Legal scales" className="" />
                    </div>
                </div>

                {/* Content Sections */}
                {contentSections.slice(1).map((section) => (
                    <div key={section.id} className={`${section.bgColor} py-8 px-6 mb-8 flex flex-col md:flex-row gap-8`}>
                        ...
                        {section.hasQuote && (
                            <div className="w-full md:w-1/3 bg-white p-6 rounded-sm shadow-sm">
                                <div className="relative">
                                    <div className="text-red-600 text-4xl absolute -left-4 -top-4">"</div>
                                    <p className="text-sm italic text-gray-800 mb-4">{section.quote}</p>
                                    <div>
                                        <p className="text-right text-sm font-semibold">{section.author}</p>
                                        <p className="text-right text-xs text-gray-600">{section.authorSubtext}</p>
                                    </div>
                                </div>
                            </div>
                        )}
