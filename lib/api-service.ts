...
export async function getConversationHistory(conversationId: string, token: string): Promise<any[]> {
    const response = await fetch(`${BACKEND_URL}/conversations/${conversationId}/history`, {
        method: "GET",
        headers: {
            Authorization: `Bearer ${token}`,
        },
...
// Upload a file
export async function uploadFile(file: File, conversationId: string, token: string): Promise<any> {
    ...
    try {
        ...
        if (!response.ok) {
            const errorData = await response.json().catch(() => ({}))
            const errorMessage = errorData.error || `Error uploading file (Status: ${response.status})`
            throw new Error(errorMessage)
        }
...
// Query documents
export async function queryDocuments(query: string, conversationId: string, token: string, topK = 5): Promise<any> {
...
export async function exportConversation(conversationId: string, format: "pdf" | "docx", token: string): Promise<Blob> {
    const response = await fetch(`${BACKEND_URL}/export/${conversationId}?format=${format}`, {
        method: "GET",
        headers: {
            Authorization: `Bearer ${token}`,
        },
...
export async function logoutUser(token: string): Promise<any> {
    try {
        ...
        if (!response.ok) {
            const errorData = await response.json().catch(() => ({}));
            const errorMessage = errorData.error || `Error logging out (Status: ${response.status})`;
            throw new Error(errorMessage);
        }
...
export async function getUserProfile(token: string): Promise<any> {
    try {
        ...
        if (!response.ok) {
            const errorData = await response.json().catch(() => ({}));
            const errorMessage = errorData.error || `Error fetching profile (Status: ${response.status})`;
            throw new Error(errorMessage);
        }
...
// Update user profile
export async function updateUserProfile(profileData: any, token: string): Promise<any> {
    try {
        ...
        if (profileData.profile_image instanceof File) {
            ...
            if (!response.ok) {
                const errorData = await response.json().catch(() => ({}));
                const errorMessage = errorData.error || `Error updating profile (Status: ${response.status})`;
                throw new Error(errorMessage);
            }
...
export async function changePassword(currentPassword: string, newPassword: string, token: string): Promise<any> {
    try {
        ...
        if (!response.ok) {
            const errorData = await response.json().catch(() => ({}));
            const errorMessage = errorData.error || `Error changing password (Status: ${response.status})`;
            throw new Error(errorMessage);
        }
...
export const updateFolder = async (folderId: number, folderName: string, token: string) => {
...
// Case APIs
export const createCase = async (caseName: string, clientName: string, token: string) => {
...
export const updateCase = async (caseId: number, data: { case_name?: string; client_name?: string }, token: string) => {
    const response = await fetch(BACKEND_URL + `/api/cases/${caseId}`, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
...
// Document APIs
export const uploadDocument = async (file: File, parentType: 'case' | 'folder', parentId: number, token: string) => {
    const formData = new FormData();
    formData.append('file', file);
...
export const getDocuments = async (parentType: 'case' | 'folder', parentId: number, token: string) => {
    const response = await fetch(BACKEND_URL + `/api/documents?parent_type=${parentType}&parent_id=${parentId}`, {
        headers: {
            'Authorization': `Bearer ${token}`
        }
    });
...
export const getArticles = async (token: string) => {
    try {
        ...
        throw new Error(error.message || "Failed to fetch articles. Please try again.");
    }
}
export const getArticleById = async (columnId: string, token: string) => {
    try {
        ...
        throw new Error(error.message || "Failed to fetch article. Please try again.");
    }
}

export const createArticle = async (title: string, content: string, token: string) => {
    try {
        const formData = new FormData();
        ...
        throw new Error(error.message || "Failed to create article. Please try again.");
    }
};
...
export const getCasesList = async (courtType: string, token: string) => {
    try {
        ...
        throw new Error(error.message || "Failed to fetch cases list. Please try again.");
    }
}
...
export const loadAmicusRecentConversations = async (token: string) => {
    try {
        ...
        throw new Error(error.message || "Failed to fetch recent conversations. Please try again.");
    }
}
